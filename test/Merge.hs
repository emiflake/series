...
xs0 :: Series Char
xs0 =
  series [(mkUTCTime i, v) | (i, v) <- [(0, 'a'), (3, 'c'), (5, 'e'), (7, 'g')]]
...
props =
  [ testProperty
      "Merge with empty on left is identity"
      prop_mergeEmptyLeftIdentity
  , testProperty
      "Merge with empty on right is identity"
      prop_mergeEmptyRightIdentity
  , testProperty
      "bounds a <> bounds b == bounds (merge a b)"
      prop_mergeBoundsDistributive
  ]
...
prop_mergeBoundsDistributive as bs =
  bounds as <> bounds bs == bounds (merge as bs)
